...
- **Testing**: Vitest + React Testing Library (unit), testcontainers (integration), Playwright (E2E)
...
### Shared Components
- `StatusBadge` — `src/components/ui/status-badge.tsx` — `<StatusBadge status={s} config={map} />`
- `EmptyState` — `src/components/ui/empty-state.tsx` — `<EmptyState heading={h} body={b} cta={{label, href}} />`
- `Money` — `src/components/ui/money-amount.tsx` — `<Money amount={number} />` (Trust Posture, tabular-nums)
- `MemberAvatar` — `src/components/ui/member-avatar.tsx` — `<MemberAvatar name={n} size="sm"|"md" />`
- `NotificationBell` — `src/components/ui/notification-bell.tsx` — embedded in Nav, polls unread count
- `TripSubNav` — `src/components/trips/trip-sub-nav.tsx` — 8 tabs (Overview through Recap), uses `usePathname()`

### Trip Sub-Pages
- All trip sub-pages live under `src/app/(app)/trips/[tripId]/` and inherit the trip layout
- Layout at `src/app/(app)/trips/[tripId]/layout.tsx` provides: auth check, breadcrumbs, trip header, status badge, TripSubNav, `<main>` wrapper
- Sub-pages should NOT wrap in `<main>` or duplicate the header — just render a `<div className="mt-6">`
- The vote board page uses `<div>` (not `<main>`) because the layout provides the wrapper

### Public Pages
- Pages outside `(app)` group (e.g., `/recaps/[slug]`) have their own minimal layout, no auth, no Nav
- Place at `src/app/recaps/[slug]/page.tsx` with a separate `layout.tsx`

### Testing
- Unit tests: `tests/unit/*.test.ts` — vitest + jsdom, no DB
- Integration tests: `tests/integration/*.test.ts` — testcontainers + real PG
- E2E tests: `tests/e2e/*.spec.ts` — Playwright + Chromium, requires running dev server + Docker
- Prefer testing pure exported functions (state machines, vote logic, party split, fee computation, swap constraints)
- Run: `pnpm test` (unit), `pnpm test:integration` (integration), `pnpm test:e2e` (E2E)

### E2E Testing (Playwright)
- Config: `playwright.config.ts` — webServer auto-starts `pnpm dev`, auth-setup project runs first
- Auth fixtures: `tests/e2e/fixtures/auth.ts` — custom `test` with `userPage`, `captainPage`, `adminPage`
- Helpers: `tests/e2e/fixtures/helpers.ts` — `login()`, `uniqueName()`, `createTrip()`
- Auth setup: `tests/e2e/auth.setup.ts` — logs in 3 test users, saves storageState to `.auth/*.json`
- Tests are sequential (`fullyParallel: false`) — they share seeded DB state
- Use `uniqueName(prefix)` for test data to avoid name collisions across runs
- Screenshots on failure, traces on first retry
- Run modes: `pnpm test:e2e` (headless), `pnpm test:e2e:headed` (visible), `pnpm test:e2e:ui` (interactive)
